...
今後の改善候補として、parser 用 fixture テストの追加と build パイプラインの安定化があります。
...
`scripts/build.mjs` は Node の `stripTypeScriptTypes` を使うため、`npm run build` 時に experimental warning が表示されます。
